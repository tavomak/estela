...
<?php if(have_posts()): while(have_posts()): the_post();?>
    ...
    <article role="article" id="post_<?php the_ID()?>">
        <header>
            ...
            <h4>
              <em>
                <span class="text-muted author"><?php _e('By', 'est-tm'); echo " "; the_author() ?>,</span>
        ...
        <section>
            ...
            <?php the_content( __( '&hellip; ' . __('Continue reading', 'est-tm' ) . ' <i class="glyphicon glyphicon-arrow-right"></i>', 'est-tm' ) ); ?>
        </section>
        <footer>
            <p class="text-muted" style="margin-bottom: 20px;">
                <i class="glyphicon glyphicon-folder-open"></i>&nbsp; <?php _e('Category', 'est-tm'); ?>: <?php the_category(', ') ?><br/>
                <i class="glyphicon glyphicon-comment"></i>&nbsp; <?php _e('Comments', 'est-tm'); ?>: <?php comments_popup_link(__('None', 'est-tm'), '1', '%'); ?>
            </p>
        </footer>
    </article>
...
<?php if ( function_exists('est_th_pagination') ) { est_th_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="older"><?php next_posts_link('<i class="glyphicon glyphicon-arrow-left"></i> ' . __('Previous', 'est-tm')) ?></li>
    <li class="newer"><?php previous_posts_link(__('Next', 'est-tm') . ' <i class="glyphicon glyphicon-arrow-right"></i>') ?></li>
  </ul>
<?php } ?>
